...
## Capa de busqueda y verificacion bibliografica

LibrerIA reutiliza la integracion existente de busqueda: Open Library es la fuente primaria y Google Books actua como fallback. Ambos clientes ya eran usados para añadir libros, realizan peticiones GET publicas, limitan resultados, aplican timeout y convierten fallos de conexion, timeout, API o payload. No se añadio ninguna clave ni un segundo cliente HTTP. En Flutter Web las llamadas siguen la misma ruta directa ya utilizada por el formulario de libros; una futura version comercial deberia mover cuotas, observabilidad avanzada y cualquier proveedor con credenciales a un backend o proxy propio.

Los resultados conservan titulo, autoria textual, editorial, ISBN, identificador externo, año, paginas y fuente. Se añadieron categorias, descripcion e idioma cuando el proveedor los devuelve. Open Library aporta `subject`, `language` y `first_sentence`; Google Books aporta `categories`, `description` y `language`. Ninguna de estas fuentes acredita de forma fiable genero de la persona autora, pertenencia y posicion exacta en serie, condicion de autoconclusivo, cierre editorial de una saga, tropo o formato concreto de la edicion buscada. La ausencia se representa como `notVerified`, nunca como `false`.

`RecommendationConstraintExtractor` activa la busqueda solo para recomendaciones explicitamente externas y acumula un conjunto limitado de restricciones desde el ultimo inicio de busqueda. Reconoce fuera de biblioteca, autoconclusivo, saga terminada, algunos generos, autoria femenina solicitada, idioma, formato, maximo de paginas y tropo evitado. No es un parser universal; expresiones no reconocidas permanecen bajo las reglas generales del prompt.

`BibliographicRecommendationService` construye una consulta minima con genero o idioma, aplica una cache en memoria de cinco minutos y no envia titulos locales, estados, valoraciones, notas ni historial. `BibliographicVerificationService` normaliza titulo, autoria e ISBN, compara localmente contra `allBooks`, descarta conflictos y solo entrega candidatos cuyas restricciones obligatorias esten `verified`. ISBN tiene prioridad; el fallback usa titulo y autoria normalizados. Todos los estados locales cuentan como presencia.

Antes del stream del LLM, `CoachRepositoryImpl` espera unicamente esta preparacion cuando la peticion es externa. `PromptBuilder` añade una seccion delimitada `Candidatos bibliográficos verificados` al contexto. El modelo debe recomendar exclusivamente desde ella y no puede presentar esos titulos como biblioteca del usuario. Sin resultados, metadatos suficientes o respuesta de la fuente, se conserva la abstencion y se indica el campo bloqueante o el fallo de busqueda.

Para una version comercial o integracion con bibliotecas publicas faltan una fuente estructurada de obras y series, autoridad de personas autoras, estado editorial mantenido, disponibilidad por catalogo y un backend que proteja credenciales, gestione cuotas, consolide evidencia entre proveedores y permita politicas de cache compartida.
